import React from "react";
import { Container, Row, Col, Button,Card, Badge, Form } from "react-bootstrap";
import "bootstrap-icons/font/bootstrap-icons.css"
import App from "../App";
import HindiLeadForm from "./HindiLeadForm";

export default function English() {
  return (
    <>    
      <section
  className="hero-section text-white text-center d-flex flex-column justify-content-center pt-5 align-items-center"
  style={{
    backgroundImage:
      "url('https://aircraftdataentry.com/assets/design_blue.db82f773.svg')",
    backgroundSize: "cover",
    backgroundPosition: "center",
    position: "relative",
  }}
  
   
>
  <div className="bg-primary bg-opacity-50 p-3 ps-5 w-100 h-100 rounded-0">
    <h1 className="display-6 p-1 fw-bold text-start">
      &nbsp;&nbsp;Want To earn Money ? <br />
      &nbsp; We have been here, <br /> &nbsp; Where are you !!
    </h1>
    <p className="lead mb-4 text-start ps-4">
      Aircraft Data Entry Private Limited is a digital platform
      <br />
      for customers to drive revenue through its services
      <br />
      which is form filling and captcha work.
    </p>

    <div className="text-start ps-4">
      <button className="btn btn-danger btn-lg rounded-5 ps-4 pe-4 fs-8 pt-1 pb-1">
        Get Started
      </button>
    </div>
    <br />

    {/* ⭐ Ratings Section */}
    <div className="pt-2 d-flex justify-content-start">
      <div className="container-fluid">
        <div className="row g-2">
          {/* Facebook */}
          <div className="col-auto">
            <div
              className="bg-light text-dark d-flex flex-column align-items-center justify-content-center rounded shadow-sm rating-card"
            >
              <img
                src="data:image/png;base64,"
                alt="Facebook"
                width="30"
                height="30"
                className="mb-1"
              />
              <div className="text-warning small">★★★★☆</div>
              <p className="small mb-0 text-center">
                4.5 / 5<br />
                800+ Reviews
              </p>
            </div>
          </div>

          {/* Instagram */}
          <div className="col-auto">
            <div
              className="bg-light text-dark d-flex flex-column align-items-center justify-content-center rounded shadow-sm rating-card"
            >
              <img
                src="https://aircraftdataentry.com/assets/google.c6fa97b5.png"
                alt="Instagram"
                width="30"
                height="30"
                className="mb-1"
              />
              <div className="text-warning small">★★★★★</div>
              <p className="small mb-0 text-center">
                4.8 / 5<br />
                2k+ Reviews
              </p>
            </div>
          </div>

          {/* LinkedIn */}
          <div className="col-auto">
            <div
              className="bg-light text-dark d-flex flex-column align-items-center justify-content-center rounded shadow-sm rating-card"
            >
              <img
                src="https://aircraftdataentry.com/assets/justdial.2d62dcc9.png"
                alt="LinkedIn"
                width="30"
                height="30"
                className="mb-1"
              />
              <div className="text-warning small">★★★★★</div>
              <p className="small mb-0 text-center">
                5.0 / 5<br />
                500+ Reviews
              </p>
            </div>
          </div>

          {/* Google */}
          <div className="col-auto">
            <div
              className="bg-light text-dark d-flex flex-column align-items-center justify-content-center rounded shadow-sm rating-card"
            >
              <img
                src="https://aircraftdataentry.com/assets/instagram.a5e8582f.png"
                alt="Google"
                width="30"
                height="30"
                className="mb-1"
              />
              <div className="text-warning small">★★★★★</div>
              <p className="small mb-0 text-center">
                4.9 / 5<br />
                1k+ Reviews
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>

  {/* ✅ Responsive Size Control */}
  <style>
    {`
      .rating-card {
        width: 100px;
        height: 120px;
      }

      @media (max-width: 576px) {
        .rating-card {
          width: 70px;
          height: 110px;
        }
      }
    `}
  </style>
</section>

                                                       {/* Card Section  */}
  <section
      className="py-5 mt-4 "
      style={{
      }}
    >
      <Container>
        <Row className="align-items-center">
          {/* 🧭 Left Side Text */}
          <Col xs={12} md={6} className="mb-4 mb-md-0">
            <p className="lead fs-3 fw-bold">
              Learn More About Our     
            </p>
            <h1 style={{ fontSize: "48px",color:"blue", fontWeight:"800"}} >Services</h1>
          </Col>




          {/* 💼 Right Side Cards */}
          <Col xs={12} md={6}>
            <div className="d-flex flex-column flex-md-row justify-content-center align-items-center gap-3">
              
              {/* Card 1 */}
              <div
                className="bg-primary p-3 rounded shadow text-white text-center d-flex flex-column align-items-center justify-content-between "
                style={{ width: "300px", minHeight: "500px" }}
              >
                <img
                  src="https://aircraftdataentry.com/assets/form_filling.d920bcd7.svg"
                  alt="Data Entry"
                  width="120"
                  className="mb-3"
                />
                <h6> Work on Computer, Laptop, Desktop or Pc Only</h6>

                <Button variant="primary" className="mt-3 px-4 rounded-pill btn-danger">
                  Form Filling
                </Button>
                <p className="text-muted">
                 Form-Filling work is a simple task. The Forms are encrypted so fill them exactly as you received them from Aircraft Data Entry Private Limited. Training. Wrok, help and everything will be provided. It is essentially a data entry job and you can do it anytime during the day. Each form is three pages, something similar like ou fill out on the internet.
                </p>
              </div>

              {/* Card 2 */}
              <div
                className="bg-info p-3 rounded shadow text-center text-white d-flex flex-column align-items-center justify-content-between"
                style={{ width: "300px", minHeight: "500px" }}
              >
                <img
                  src="https://aircraftdataentry.com/assets/captcha_filling.38d414b2.svg"
                  alt="Form Filling"
                  width="120"
                  className="mb-3"
                />
                <h6> Work on Mobile, Computer, Laptop, Desktop or Pc Only </h6>
                <Button variant="success" className="mt-3 px-4 rounded-pill btn-danger">
                  Captcha
                </Button>
                <p className="text-muted">
                Here the work is quite simple. Just log in to your account with your username & password and after visiting the work area, you can see the CAPTCHA image being loaded. Traning , work, and everything will be provided. What you are required to do is to read the characteristics in the image code & simply type the right code & click submit button. For every correct captcha, you will get paid.
                </p>
              </div>
            </div>
          </Col>
        </Row>
      </Container>
    </section>



                                                   {/* ===== How It Works Section with Left Circles ===== */}
<section className="py-5 bg-info text-white " >
  <Container>
    <Row className="align-items-center">
      {/* Left Side */}
      <Col xs={12} md={6} className="mb-5 mb-md-0 text-center">
        <div className="d-flex flex-column align-items-center gap-4">
          <div>
            <h2 className="fw-bold fs-1">How it Works?</h2>
            <h4 className="fw-bold fs-1 text-danger">Form Filling</h4>
          </div>
          <div className="ratio ratio-16x9 w-100">
           <iframe width="560" height="315" src="https://www.youtube.com/embed/_RKW8IZz8yE?si=-N4VQfbTOhaO6riK" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen>
         </iframe>
          </div>
          <div className="d-flex flex-column align-items-center gap-3 w-100">
            <Button variant="danger" className="rounded-pill px-4"> 
              <img
            src="https://aircraftdataentry.com/assets/desktop.33bf672e.png"
            alt="Google Logo"
            width="20"
            height="20"
            className="mb-1"
          />&nbsp;&nbsp;
              Form Filling Demo Work
            </Button>
            <Button variant="danger" className="rounded-pill px-4 ">
                <img
            src="data:image/png;base64,"
            alt="Google Logo"
            width="20"
            height="20"
            className="mb-1"
          />&nbsp;&nbsp;
              Form Filling Training Video
            </Button>
          </div>
        </div>
      </Col>

      {/* Right Side - Divs with Circles Side by Side */}
      <Col xs={12} md={6}>
        <div className="d-flex flex-column gap-3">
          {[
            {
              
              desc1: "We are am MCA Register companty working here for more than 11 yeare with more than 13 Thousanf satisfied users.",
              // desc2: "Fill accurately and get paid per form."
            },
            {
              desc1: "Once the online registration is complete, 300 forms will be assigned to the customer along with the software and full training videos details where it needs to filled.",
    
            },
            {
              desc1: "After the successful completion of 300 forms, the amount will be credited to the custome's bank account with as-per-plan rate.",            
            },
            {             
              desc1: "92% accuray must be maintained while forms. because data is encrypted. Which means in 300 forms no more than 24 forms should be wrongly filled. (One form means three pages).",
            },
            {             
              desc1: "Once per the first set of 300 forms is filled successfully with the required quality, the second set will be provided at a high rate as per the plans. If the customer completed earning an amount of 35000 thousand rupees, the full registration amount will be refunded back.",
            },
            {           
              desc1: "We provide all support and help to users when they face any problem at work. While they doing work we provide data and we need only work from as per their own time thats's it. And they get paid as per the plan.",
             
            }
          ].map((feature, index) => (
            <div key={index} className="d-flex gap-3 align-items-center">
              {/* Circle */}
              <div
                className="d-flex justify-content-center align-items-center border border-white rounded-circle bg-danger text-white flex-shrink-0"
                style={{ width: "30px", height: "30px", fontSize: "1.2rem" }}
              >
                {index + 1}
              </div>

              {/* Main Div */}
              <div className="bg-primary border border-white rounded shadow text-start p-3 flex-grow-1">
                <h6 className="fw-bold mb-1 text-white">{feature.title}</h6>
                <p className="small mb-0 text-light">{feature.desc1}</p>
                <p className="small mb-0 text-light">{feature.desc2}</p>
              </div>
            </div>
          ))}
        </div>
      </Col>
    </Row>
  </Container>
</section> 

                                            {/* ===== How It Works Section with Left Circles on Left Side 2 ===== */}
<section className="py-5 bg-info text-white"><br/> <br/> <br/>
  <Container>
    <Row className="align-items-center">
      {/* Left Side - Features with Circles */}
      <Col xs={12} md={6} className="mb-5 mb-md-0">
        <div className="d-flex flex-column gap-3">
          {[
            {
              desc1: "For Captcha works, there is no target, accuracy, deduction, or time limit.",
            },
            {
              desc1: "Do work from anywhere at any time (24/7) through your mobile / computer / laptop.",
            },
            {
              desc1: "Your daily earnings will be directly transferred to your bank account.",
            },
            {             
              desc1: "One- time payment, lifetime work, and earnings.",
            },
            {             
              desc1: "Your security deposit amount will be refunded once you earn Rs. 35000 thousand rupees.",
            },
            {           
              desc1: "We are an MCA Register company working here for more than 11 years with more than 13 Thousand satisfied users. We provide all support and help to users when they face any problm in work while they",
            }
          ].map((feature, index) => (
            <div key={index} className="d-flex gap-3 align-items-center">
              {/* Circle */}
              <div
                className="d-flex justify-content-center align-items-center border border-white rounded-circle bg-danger text-white flex-shrink-0"
                style={{ width: "30px", height: "30px", fontSize: "1.2rem" }}
              >
                {index + 1}
              </div>

              {/* Main Div */}
              <div className="bg-primary border border-white rounded shadow text-start p-3 flex-grow-1">
                <p className="small mb-0 text-light">{feature.desc1}</p>
              </div>
            </div>
          ))}
        </div>
      </Col>

      {/* Right Side - Headings + Video + Buttons */}
      <Col xs={12} md={6} className="text-center">
        <div className="d-flex flex-column align-items-center gap-4">
          <div>
            <h2 className="fw-bold fs-1 text-danger">Captcha</h2>
          </div>
          <div className="ratio ratio-16x9 w-100">
         <iframe width="560" height="315" src="https://www.youtube.com/embed/_RKW8IZz8yE?si=-N4VQfbTOhaO6riK" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen>
         </iframe>
          </div>
          <div className="d-flex flex-column align-items-center gap-3 w-100 ">
            <Button variant="danger" className="rounded-pill px-4 "> 
              <img
                src="https://aircraftdataentry.com/assets/desktop.33bf672e.png"
                alt="Google Logo"
                width="20"
                height="20"
                className="mb-1"
              />&nbsp;&nbsp;
              Captcha Demo Work 
            </Button>
            <Button variant="danger" className="rounded-pill px-4 w-40">
              <img
                src="data:image/png;base64,"
                alt="Google Logo"
                width="20"
                height="20"
                className="mb-1"
              />&nbsp;&nbsp;
              Captcha Demo App
            </Button>
            <Button variant="danger" className="rounded-pill px-3 py-2 ">
              <img
                src="data:image/png;base64,"
                alt="Google Logo"
                width="20"
                height="20"
                className="mb-1"
              />&nbsp;&nbsp;
              Captcha Training Video 
            </Button><br/><br/><br/><br/>
          </div>
        </div>
      </Col>
    </Row>
  </Container>
</section>

                                                {/* Professional priceing card section  */}
   
<section
  className="py-5"
  style={{
    background: "linear-gradient(180deg, #3584daff 100%)",
  }}
>
  <h1 className="text-center fs-1 fw-bold text-white py-5"> Our Plans </h1>
  <Container>
    <Row className="g-4 justify-content-center">
      {[
        {
          title: "CAPTCHA PLAN ",
          price: "3999",
          description: "Including GST - Service + Service Charge",
          tag: "Refundable",
          note: "T & C Applied",
          info: [
            "In this plan you get Captcha work", 
            "Simple data entry-based tasks with accuracy bonus Flexible working hours from anywhere Instant online support available"
          ],
        },
        {
          title: "FORM FILLING PLAN ",
          price: "4999",
          description: "Including GST + Service Charge",
          tag: "Refundable",
          note: "T & C Applied",
          info: [
            "In this plan you will get Form Filling Work ",
            "Form Filling info- You Will get 300 forms. first 300 Forms rate will be 12 per Form And After maintaining 92% quality for the next 300 forms your rates will increase by 30 Rs per Form. ",
        
          ],
        },
        {
          title: "CAPTCHA PLAN ",
          price: "5999",
          description: "Including GST + Service Charge",
          tag: "Refundable",
          note: "T & C Applied",
          info: [
            "In this plan you will get Captcha Work.",
            "Captcha info- For captcha. type 999 correct words you will get 1 dollar.(1 dollar means 75 Rs)",
          
          ],
        },
        {
          title: "FORM FILLING & CAPTCHA PLAN ",
          price: "6999",
          description: "Including GST + Service Charge",
          tag: "Refundable",
          note: "T & C Applied",
          info: [
            "In this plan you will get both the works form filling & captcha ",
            " Form Filling info- You will get the first 300 forms rate is 16 Rs per form. And after maintaining 92% quality fot the 300 forms your rates will increase by 60 Rs Per form ",
            "Captcha info- For captcha, type 2499 correct words and you will get 1 dollar (1 dollar means 75 Rs)."
           
          ],
        },
        {
          title: "FORM FILLING & CAPTCHA PLAN",
          price: "8999",
          description: "Including GST  + Service Charge",
          tag: "Refundable",
          note: "T & C Applied",
          info: [
            "In this plan you will get both the works form filling and captcha ",
            "form Filling info- You will get 300 forms. First 300 forms rate will be 25 Rs per form. And after maintaining 92% quality for the next 300 forms your rates will increase by 120 Rs per form",
            "Captcha info- For Captcha type 1999 correct words and you will get 1 dollar.(1 dollar means 75 Rs.)"
          ],
        },
      ].map((plan, index) => (
        <Col key={index} xs={12} sm={6} md={4} lg={4}>
          <Card
            className="h-100 shadow border-0 bg-white text-dark"
            style={{
              borderRadius: "15px",
              transition: "transform 0.3s ease",
            }}
          >
            <Card.Body style={{ background: "#d6f3f3ff",borderRadius:"25px"}} className="d-flex flex-column p-4 ">
              {/* Header */}
              <div className="text-center border-bottom pb-3 mb-3">
                <h4 style={{ color: "#2e8cf0ff" }} className="fw-bold">{plan.title}</h4>
                <h5 style={{ color: "#55718eff" }} className="fw-bold  mb-2 fs-1">
                  RS. {plan.price}/-
                </h5>
                <p style={{ fontSize: "0.7rem" }}  className="small text-muted mb-1 fs-">{plan.description}</p>
                <span className="badge bg-danger text-white px-3 py-2 fw-semibold ">
                  {plan.tag}
                </span>
                <p style={{ fontSize: "0.7rem" }}  className="small mt-2 ">{plan.note}</p>
              </div>

              {/* Content */}
              <div className="mb-3">
                {plan.info.map((item, i) => (
                  <p key={i} style={{ fontSize: "0.7rem" }}  className="mb-1  small">
                    <i className="bi bi-check2-circle text-primary me-2"></i>
                    {item}
                  </p>
                ))}
              </div>

              {/* Button */}
              <div className="mt-auto text-center">
                <Button
                  variant="danger "
                  className="fw-semibold px-4 py-2 rounded-pill"
                >
                  Subscribe
                </Button>
              </div>

              {/* Note with Background Image */}
              <div
                className="mt-3 p-2 text-center rounded text-black small bg-primary"
                style={{
                  backgroundImage:
                    "url('https://aircraftdataentry.com/assets/design_light_blue_left.13e88b1e.svg')",
                  backgroundSize: "cover",
                  backgroundPosition: "center",
                }}
              >
               <p style={{ fontSize: "0.7rem" }} className="mb-0"> Note: This Security Deposit fee for training and software. and it is refundable after completing 35000 Rs for work  </p>
              </div>
            </Card.Body>
          </Card>
        </Col>
      ))}
    </Row>
  </Container>
</section>
                                                    {/* About Us   */}


<section className="py-5">
  <div className="container">
    <div className="row align-items-center justify-content-center g-4">
      {/* Left Side */}
      <div className="col-12 col-md-5 text-center">
        <h2 className="fw-bold mb-4 text-primary display-5">About Us</h2>
        <img
          src="https://aircraftdataentry.com/assets/about_us_image.af9576e5.svg"
          alt="About Illustration"
          className="img-fluid rounded"
          style={{ maxHeight: "300px", objectFit: "contain" }}
        />
      </div>

      {/* Right Side */}
      <div className="col-12 col-md-6">
        <div
          className="p-4 bg-primary text-white shadow position-relative mx-auto"
          style={{
            width: "90%",
            minHeight: "480px",
            border: "6px solid #0dcaf0",
            borderTopLeftRadius: "45px",
            borderBottomRightRadius: "15px",
          }}
        >
          <p className="mb-3 fs-6 lh-base">
            Aircraft Data Entry Private Limited is a digital platform for customers to
            drive revenue through its services, including form filling and captcha work.
            Just over a couple of years ago, we became fascinated by the idea of
            discovering a company that provides customers with a revenue generation
            platform with minimal effort in return. Since 2009, Aircraft Data Entry
            Private Limited has been serving end-users with an easy and accessible
            approach to earning. Over the years, we have successfully gathered over
            13,000 customers who have been part of our engagement.
            We are inspired by go-getters, achievers, and dreamers — and our platform
            serves as a tool to achieve milestones and generate income efficiently. We
            believe in growing together with accuracy, trust, and transparency. Our
            company is fully regulated with all required documentation, ensuring your
            payments are always secure with us.
          </p>

          {/* Note at Bottom */}
          <div className="bg-danger text-white text-center rounded py-2 px-3 fw-semibold position-absolute bottom-0 start-0 w-100">
            <small>Grab the opportunity, People!</small>
          </div>
        </div>
      </div>
    </div>
  </div> <br /><br /><br /><br /><br /><br />


                                                          {/* About Us 2 */}


    <div className="container">
    <div className="row align-items-center justify-content-center g-4">
            {/* Left Side */}
      <div className="col-12 col-md-6">
  <div
    className="p-4 bg-primary text-white shadow position-relative mx-auto"
    style={{
      width: "80%",          // decreased width   
      minHeight: "300px",    // decreased height
      border: "4px solid #0dcaf0",
      borderTopLeftRadius: "40px",
      borderBottomRightRadius: "15px",
    }}
  >
    <p className="mb-3 fs-6 lh-base">
      Online form filling and captcha jobs are one of the easiest work-from-home jobs. Companies are willing to pay good money to have you fill in forms because there are so many of them to fill. The internet is saturated with content and I bet that you have filled out a form or two in a day. So, not only is the task easy but it is also a great & easy way to earn money fast.
    </p>

    {/* Note at Bottom */}
    <div className="bg-danger text-white text-center rounded py-2 px-3 fw-semibold position-absolute bottom-0 start-0 w-100">
      <small>You save our time, and time = money, so we are willing to pay you for the privilege of having more time.</small>
    </div>
  </div>
</div>


      {/* Right Side */}
      <div className="col-12 col-md-5 text-center">
        <h2 className="fw-bold mb-4 text-primary display-5">Why Us</h2>
        <img
          src="https://aircraftdataentry.com/assets/why_us.b025aab6.svg"
          alt="About Illustration"
          className="img-fluid rounded"
          style={{ maxHeight: "300px", objectFit: "contain" }}
        />
      </div>
    </div>
  </div>
</section>



                                                    {/* Testmonials  */}
<section
  className="py-5"
  style={{
    backgroundColor: "#007bff",
  }}
>
  <h1 className="text-center text-white fw-bold"> Testimonials </h1> <br /><br />
  <Container>
    <Row className="g-4 justify-content-center">
      {[
        {
          name: "Choti Das",
          stars: 5,
          text: "For the last two years, I have been employed by this organization. Making additional money at home is a fantastic idea. Money is no issue for me. For Aircraft Data Entry, it's the greatest firm out there.",
        },
        {
          name: "Khushi Sehna",
          stars: 5,
          text: "I've just recently begun working with this company. The staff is really kind and encouraging. It's one of the most successful companies — Aircraft Data Entry offers a nice working atmosphere and respects its employees properly.",
        },
        {
          name: "Sunil Suhu",
          stars: 5,
          text: "Aircraft Data Entry is a well-known name in the industry today. The staff is kind and helpful, and they take the time to assist everyone. I really like their work ethic.",
        },
        {
          name: "Kumkum Das",
          stars: 5,
          text: "For the last two years, I've worked for this firm. You can make money from the comfort of your own home. Among the best data entry companies for aircraft.",
        },
      ].map((item, index) => (
        <Col key={index} xs={12} md={5} className="d-flex justify-content-center">
          <div
            className="p-4 bg-white text-dark shadow d-flex flex-column  align-items-center text-center"
            style={{
              width: "100%",
              maxWidth: "360px",
              minHeight: "300px",
              border: "3px solid #0dcaf0",
              borderTopLeftRadius: "40px",
              borderBottomRightRadius: "40px",
            }}
          >
            {/* Stars at Top */}
            <div className="mb-2">
              {[...Array(item.stars)].map((_, i) => (
                <i key={i} className="bi bi-star-fill text-warning me-1"></i>
              ))}
            </div>

            {/* Name */}
            <h5 className="fw-bold text-primary mb-3">{item.name}</h5>

            {/* Paragraph */}
            <p
              className="mb-0 fst-italic"
              style={{ fontSize: "1.05rem", lineHeight: "1.6" }}
            >
              {item.text}
            </p>
          </div>
        </Col>
      ))}
    </Row>
  </Container>
</section>


                                                    {/* PDf download Section  */}

<section
  className="py-5"
  style={{
    backgroundColor: "#f8f9fa",
    position: "relative",
    overflow: "hidden",
  }}
>
  <Container>
    <h1 className="text-center text-primary fw-bold pb-4"> Documents</h1>
    {/* 📄 Two Rows with 2 Cards Each */}
    <Row className="justify-content-center mb-4 g-4">
      {/* Card 1 */}
      <Col xs={12} md={6} lg={5} className="d-flex justify-content-center">
        <div
          className="bg-white shadow rounded p-3 text-center"
          style={{ width: "340px", minHeight: "300px" }}
        >
          <img
            src="https://captchas-new.s3.ap-south-1.amazonaws.com/aircraft-documents/1.jpeg"
            alt="Document 1"
            className="rounded mb-3"
            style={{ width: "100%", height: "160px", objectFit: "cover" }}
          />
          <h6 className="fw-bold mb-2">Document 1</h6>
          <p className="text-muted small mb-3">Download your first file.</p>
          <Button
            variant="primary"
            size="sm"
            onClick={() => {
              const link = document.createElement("a");
              link.href = "/sample1.pdf";
              link.download = "sample1.pdf";
              link.click();
            }}
          >
            MCA Certificate 📥
          </Button>
        </div>
      </Col>

      {/* Card 2 */}
      <Col xs={12} md={6} lg={5} className="d-flex justify-content-center">
        <div
          className="bg-white shadow rounded p-3 text-center"
          style={{ width: "340px", minHeight: "300px" }}
        >
          <img
            src="https://captchas-new.s3.ap-south-1.amazonaws.com/aircraft-documents/2.jpeg"
            alt="Document 2"
            className="rounded mb-3"
            style={{ width: "100%", height: "160px", objectFit: "cover" }}
          />
          <h6 className="fw-bold mb-2">Document 2</h6>
          <p className="text-muted small mb-3">Download your second file.</p>
          <Button
            variant="success"
            size="sm"
            onClick={() => {
              const link = document.createElement("a");
              link.href = "/sample2.pdf";
              link.download = "sample2.pdf";
              link.click();
            }}
          >
            GST Certificate 📥
          </Button>
        </div>
      </Col>
    </Row>

    <Row className="justify-content-center mb-5 g-4">
      {/* Card 3 */}
      <Col xs={12} md={6} lg={5} className="d-flex justify-content-center">
        <div
          className="bg-white shadow rounded p-3 text-center"
          style={{ width: "340px", minHeight: "300px" }}
        >
          <img
            src="https://captchas-new.s3.ap-south-1.amazonaws.com/aircraft-documents/3.jpeg"
            alt="Document 3"
            className="rounded mb-3"
            style={{ width: "100%", height: "160px", objectFit: "cover" }}
          />
          <h6 className="fw-bold mb-2">Document 3</h6>
          <p className="text-muted small mb-3">Download your third file.</p>
          <Button
            variant="warning"
            size="sm"
            onClick={() => {
              const link = document.createElement("a");
              link.href = "/sample3.pdf";
              link.download = "sample3.pdf";
              link.click();
            }}
          >
            ISO Certificate 📥
          </Button>
        </div>
      </Col>

      {/* Card 4 */}
      <Col xs={12} md={6} lg={5} className="d-flex justify-content-center">
        <div
          className="bg-white shadow rounded p-3 text-center"
          style={{ width: "340px", minHeight: "300px" }}
        >
          <img
            src="https://captchas-new.s3.ap-south-1.amazonaws.com/aircraft-documents/4.jpeg"
            alt="Document 4"
            className="rounded mb-3"
            style={{ width: "100%", height: "160px", objectFit: "cover" }}
          />
          <h6 className="fw-bold mb-2">Document 4</h6>
          <p className="text-muted small mb-3">Download your last file.</p>
          <Button
            variant="danger"
            size="sm"
            onClick={() => {
              const link = document.createElement("a");
              link.href = "/sample4.pdf";
              link.download = "sample4.pdf";
              link.click();
            }}
          >
            DIN Certificate 📥
          </Button>
        </div>
      </Col>
    </Row>
    <br />

    {/* 🎥 Heading + Medium YouTube Video */}
    <Row className="justify-content-center text-center">
      <Col xs={12}>
        <h2 className="fw-bold display-4 mb-5 text-primary">Video Documents </h2>

        <div
          className="mx-auto"
          style={{
            maxWidth: "700px",
            aspectRatio: "16/9",
          }}
        >
          <iframe
            width="90%"
            height="90%"
            src="https://www.youtube.com/embed/dQw4w9WgXcQ"
            title="YouTube Video"
            allowFullScreen
          ></iframe>
        </div>
      </Col>
    </Row>


    {/* 📞 New Contact Section Below Video */}
    <Row className="align-items-center justify-content-center mt-5 g-4">
      {/* Left Side Image */}
      <Col xs={12} md={6} className="text-center">
        <img
          src="https://aircraftdataentry.com/assets/contact.6f65b4b0.svg"
          alt="Contact Us"
          className="img-fluid rounded"
          style={{ maxHeight: "250px", objectFit: "cover" }}
        />
        <h1 className="pt-5 text-primary fw-bold"> Contact Us </h1>
      </Col>
<HindiLeadForm/>
    </Row>
  </Container>
</section>


                             {/* Contact Details Section */}
<section id="contact-session"
  className="py-5 text-white"
  style={{ backgroundColor: "#5468ff", overflow: "hidden" }}
>
  <Container>
    <Row className="gy-4">
      {/* Contact Us */}
      <Col xs={12} md={4}>
        <h5 className="fw-bold mb-3">Contact Us</h5>
        <p className="mb-1">
          <strong>Email:</strong>{" "}
          <a
            href="mailto:demo@aircraftdataentry.com"
            className="text-white text-decoration-none"
          >
            lokhandeshekhar122@gmail.com
          </a>
        </p>

        <h6 className="fw-bold mt-4 mb-3">Get In Touch</h6>
        <div className="d-flex align-items-center bg-white rounded-pill p-2 shadow-sm">
          <a
            href="#"
            className="d-flex align-items-center justify-content-center mx-2"
          >
            <img
              src="https://cdn-icons-png.flaticon.com/512/300/300221.png"
              alt="Google"
              width="28"
              height="28"
            />
          </a>
          <div className="vr"></div>
          <a
            href="#"
            className="d-flex align-items-center justify-content-center mx-2"
          >
            <img
              src="https://cdn-icons-png.flaticon.com/512/733/733547.png"
              alt="Facebook"
              width="28"
              height="28"
            />
          </a>
          <div className="vr"></div>
          <a
            href="#"
            className="d-flex align-items-center justify-content-center mx-2"
          >
            <img
              src="https://cdn-icons-png.flaticon.com/512/2111/2111463.png"
              alt="Instagram"
              width="28"
              height="28"
            />
          </a>
          <div className="vr"></div>
          <a
            href="#"
            className="d-flex align-items-center justify-content-center mx-2"
          >
            <img
              src="https://cdn-icons-png.flaticon.com/512/1384/1384060.png"
              alt="YouTube"
              width="28"
              height="28"
            />
          </a>
          <div className="vr"></div>
          <a
            href="#"
            className="d-flex align-items-center justify-content-center mx-2"
          >
            <img
              src="https://aircraftdataentry.com/assets/justdial.2d62dcc9.png"
              alt="JustDial"
              width="28"
              height="28"
            />
          </a>
        </div>
      </Col>

      {/* Legal */}
      <Col xs={12} md={4}>
        <h5 className="fw-bold mb-3">Legal</h5>
        <p className="mb-2">
          <a href="#" className="text-white text-decoration-none">
            Terms & Conditions
          </a>
        </p>
        <p className="mb-2">
          <a href="#" className="text-white text-decoration-none">
            Privacy Policy
          </a>
        </p>
        <p className="mb-0">
          <a href="#" className="text-white text-decoration-none">
            Refund Policy
          </a>
        </p>
      </Col>

      {/* Address */}
      <Col xs={12} md={4}>
        <h5 className="fw-bold mb-3">Address</h5>
        <p className="mb-2">
          C-2 Manik Chembar, Janta Bazar Chowk,
          <br />
          Rajarampuri, Kolhapur. Pin 416008
        </p>
        <p className="mb-1">CIN NO:- U72900PN2019PTC186857</p>
        <p className="mb-0">GSTN - 27AAHCT5296R1Z9</p>
      </Col>
    </Row>
  </Container>
</section>





      <footer className="text-center py-3 bg-info text-light">
        Powered by TechAircraft Solution Private Limited
      </footer>
    </>
  );
}
